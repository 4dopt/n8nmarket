...
type View = 'home' | 'pricing' | 'documentation';
  ...
  const filteredWorkflows = useMemo(() => {
    return WORKFLOWS.filter((workflow) => {
      ...
      const matchesPlatform = filters.selectedPlatforms.length === 0 || filters.selectedPlatforms.some(platform => {
        return workflow.integrations.some(integration => 
          integration.toLowerCase().includes(platform.toLowerCase())
        );
      });
        ...
        <div className="fixed inset-0 z-[60] lg:hidden">
          ...
          <div className="absolute inset-y-0 right-0 w-full max-w-[320px] bg-[#F5F7FF] shadow-2xl animate-in slide-in-from-right duration-300 flex flex-col">
            ...
            <div className="p-5 bg-white border-t border-slate-200 shrink-0">
              <button 
                onClick={() => setShowMobileFilters(false)}
                className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-3.5 rounded-xl transition-colors shadow-lg shadow-indigo-200"
              >
                Show {filteredWorkflows.length} Results
              </button>
            </div>
          </div>
        </div>
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-2 md:grid-cols-4 gap-8">
      <div>
        ...
        <ul className="space-y-2 text-sm text-slate-500">
          <li><button onClick={() => onNavigate('home')} className="hover:text-indigo-600">Workflows</button></li>
          <li><button onClick={() => onNavigate('pricing')} className="hover:text-indigo-600">Pricing</button></li>
      ...
      <div>
        ...
        <ul className="space-y-2 text-sm text-slate-500">
          <li><button onClick={() => onNavigate('documentation')} className="hover:text-indigo-600">Documentation</button></li>
